...
*/
 ...
 /*
// Define methods to access graph data
G.getNodes = function() {
...
G.successors = function(nodeId) {
  return this.getEdges().filter(edge => edge[0] === nodeId).map(edge => edge[1]);
};

G.predecessors = function(nodeId) {
  return this.getEdges().filter(edge => edge[1] === nodeId).map(edge => edge[0]);
};
*/
...
function countEdgeCrossings(G, pos) {
  let crossings = 0;
  //const edges = G.edges();
...
function chooseBestShiftDirection(G, pos, nodeToMove, shiftAmount) { 
...
// WORKS FOR G WITH THIS FORMAT
// Define node data without positions
const data = {
  nodes: [{ id: 'n1' }, { id: 'n2' }, { id: 'n3' }],
  edges: [
    { id: 'e1', source: 'n3', target: 'n2' },
    { id: 'e2', source: 'n2', target: 'n1' }
  ]
};

// Define a separate dictionary for positions
const positions = {
  'n1': { x: 100, y: 100 },
  'n2': { x: 200, y: 200 },
  'n3': { x: 300, y: 300 }
...
function samiLayout0(G: any, entryNodes: string[], exitNodes: string[], ogma:Ogma) {
    ...
    //G = {nodes:[id], edges:[id, source, target]}
    ...
    ogma.setGraph(G).then(() => {
      // Apply positions from the dictionary
